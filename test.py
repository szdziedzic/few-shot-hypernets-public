...
def single_test(params):
    ...
    if params.method in [
        ...
        if isinstance(model, (MAML, BayesHMAML, HyperMAML)):
            acc_mean, acc_std, eval_time, acc_wc_mean, acc_wc_std, *_ = model.test_loop(
    ...
    return acc_mean, acc_wc_mean, eval_time
...
def perform_test(params):
    ...
    for i in range(seed, seed + repeat):
        if seed != 0:
            ...
            _set_seed(0)
        acc, acc_wc, test_time = single_test(params)
        accuracy_list.append(acc)
        accuracy_wc_list.append(acc_wc)
    ...
    mean_acc_wc = np.mean(accuracy_wc_list)
    std_acc_wc = np.std(accuracy_wc_list)
    print("-----------------------------")
    print(
    ...
    return {
        ...
        "time_std": std_time,
        "n_seeds": repeat,
        "accuracy_wc_mean": mean_acc_wc,
        "accuracy_wc_std": std_acc_wc,
    }
