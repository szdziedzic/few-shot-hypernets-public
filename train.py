                    ...
                    if params.hm_set_forward_with_adaptation:
                        for i in range(params.hn_val_epochs + 1):
                            if i != 0:
                                ...
                                ] = metrics_per_epoch[f"accuracy/val_support_max@-{i}"][
                                    -1
                                ]
                            max_acc_adaptation_dict[
    ...
    for epoch in range(start_epoch, stop_epoch):
        ...
        if (epoch % params.eval_freq == 0) or epoch in [
            ...
            try:
                ...
                acc, acc_wc = model.test_loop(val_loader)
                test_loop_metrics = dict()
            print("acc", acc)
            print("acc_wc", acc_wc)
            print("test_loop_metrics", test_loop_metrics)
            print(
...
def plot_metrics(
    metrics_per_epoch: Dict[str, Union[List[float], float]], epoch: int, fig_dir: Path
):
...
def get_scheduler(params, optimizer, stop_epoch=None) -> lr_scheduler._LRScheduler:
    if params.lr_scheduler == "multisteplr":
        ...
        return lr_scheduler.MultiStepLR(optimizer, milestones=milestones, gamma=0.3)
...
if __name__ == "__main__":
    ...
    if params.method in ["baseline", "baseline++"]:
        ...
        base_datamgr = SetDataManager(
            image_size, **train_few_shot_params
        )  # n_eposide=100
        base_loader = base_datamgr.get_data_loader(base_file, aug=params.train_aug)
